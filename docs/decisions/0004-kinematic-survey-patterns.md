...
| **Relocatable base / walking-arm logic** (undock → move → dock; power/data via anchored end) | Canadarm2, ERA | **+** Satisfies R2 with ≥2 docks. **–** Flight-symmetric dual LEE is heavy/costly if copied literally. | **Pattern-only** — keep base-change sequence and dual-role end *logic*; scale interfaces to consumer docks |
