...
## 1. Shared Assumptions and Context for All Scoring
...
> **Note on Methodology:** Numeric scores below are relative comparative judgments to help rank concepts, not laboratory metrics. Static masses, link lengths, and joint torques represent an illustrative order-of-magnitude model used for sizing checks rather than fully simulated or measured physical properties.

---

## 2. K5.1 Scorecard

Each scheme is scored on a scale of **1 to 5** (where 5 represents the lowest risk / highest alignment with project goals, and 1 represents a disqualifying risk or veto failure).

*The numeric scores are relative qualitative rankings to guide architectural decisions rather than precise mathematical outputs.*

| Evaluation Criterion | S1 (Modular In-Joint) | S2 (Cascaded Mass Bias) | S3 (Symmetric Dual-Ended) |
| :--- | :---: | :---: | :---: |
| **DFAA (Design for Autonomous Assembly)** | **4** / 5<br>Clean boundaries; simple joint swaps; internal wiring routing is complex. | **4** / 5<br>Belt maintenance is an accepted trade-off for lower inertia; channels are agent-accessible. | **4** / 5<br>Symmetric design reduces unique parts; identical end clusters are easy to assemble. |
...
| **BOM Risk** | **4** / 5<br>6 actuator pods; lower torque distal motors are highly affordable. | **4** / 5<br>6 motors; timing belts and pulleys are extremely low-cost COTS parts. | **2** / 5<br>Requires 7 proximal-class actuator pods; high risk of exceeding the N7 budget limit. |
...
| **Fit ADR-0003** | **4** / 5<br>Fits reach, payload, and cost targets; transit battery fits on Link 1. | **5** / 5<br>Excellent fit; maximizes N3 payload and N5 torque/speed margins; default policy. | **2** / 5<br>High risk of exceeding N7 BOM limit; high terminal mass may erode continuous payload margin under assumed masses. |
| **QUALITATIVE RANK** | **Rank 2 (Strong Backup)** | **Rank 1 (Primary Candidate)** | **Rank 3 (Deferred)** |

---

## 3. K5.2 Order-of-Magnitude Analysis

### 3.1 Static Load and Torque Calculations (Illustrative Physical Baseline)
To ground the torque checks, we define a hypothetical mass model for a $0.5 \text{ kg}$ payload and $0.8 \text{ m}$ maximum reach:
...
3.  **S3 (Symmetric Dual-Ended):**
    *   S3 must carry an identical actuator cluster at its active distal end.
    ...
    *   With a safety factor, the base shoulder joint J2 requires **$\ge 25\text{--}30 \text{ N}\cdot\text{m}$** continuous. This poses a significant risk of pushing proximal actuators to their thermal limits and exceeding the N7 budget target due to the need for larger, high-performance motors.
...
*   **Rough Torque Check (Proximal vs. Distal):**
    *   Distal Wrist (J4, J5, J6): Carry local, small distal-class BLDC motors with planetary gearboxes. Combined wrist assembly weight $\approx 0.35 \text{ kg}$.
    ...
    *   Distal continuous torque target of $1\text{--}3 \text{ N}\cdot\text{m}$ provides a solid safety margin.
...
*   **Rough Torque Check (Proximal vs. Distal):**
    *   Forearm roll (J4), wrist pitch (J5), and wrist roll (J6) actuators are grouped into a "motor pack" located at the very base of Link 3 (elbow joint). Wrist weight drops to an illustrative $0.12 \text{ kg}$.
    *   The distal wrist torque of $1\text{--}3 \text{ N}\cdot\text{m}$ is maintained via short timing belts (an illustrative implementation pattern).
    *   The J3 elbow pitch actuator is located at the base of Link 2 (shoulder joint), reducing Upper Arm dynamic inertia.
    *   J3 continuous torque is $10\text{--}15 \text{ N}\cdot\text{m}$. Driven by a proximal BLDC pod via an illustrative $3:1$ secondary timing belt reduction, the physical actuator needs to output only $3\text{--}5 \text{ N}\cdot\text{m}$ locally, reducing actuator size and thermal load.
*   **Dock Spacing vs. Dual-Dock Reach:**
    ...
    *   This spacing fits into a single-cluster desktop footprint, allowing Dock A to service the printer bed while Dock B directly overlooks the assembly table and tool bay.
...
*   **Printable Segments vs. Size Limit ($\le 175\text{--}180 \text{ mm}$):**
    ...
    *   Each link is segmented into 2 structural tubes of $175 \text{ mm}$ each. Fits the print bed limit, but the higher structural loading of S3's dual-ended walking may require a higher-stiffness print strategy or thicker structural walls, increasing dead-weight.
*   **Rough Torque Check (Proximal vs. Distal):**
    ...
    *   S3 must carry an estimated $3 \text{ kg}$ of total arm weight. When rooted at End-A, the joint J2 must lift Link 1, J4, Link 2, the heavy End-B cluster, and the $0.5 \text{ kg}$ payload.
    *   Required torque at J2 under full extension: $\approx \mathbf{16.3 \text{ N}\cdot\text{m}}$ static. Adding dynamic acceleration spikes, the system may require $\ge 25\text{--}30 \text{ N}\cdot\text{m}$ of actuator capability, creating a thermal risk for 3D-printed/COTS modular gearboxes.
*   **Dock Spacing vs. Dual-Dock Reach:**
    ...
    *   While S3 has the largest step reach, it carries a significant weight, thermal, and cost risk.
...
*   **Why it is deferred/rejected for Phase 1:** S3 represents an elegant, symmetric "pure walking" concept (Canadarm-style). However, carrying heavy, high-torque proximal actuators on both ends is highly inefficient for this scale. The active end-effector end becomes excessively heavy, which dramatically inflates the shoulder torque requirements to lift the arm itself. Building S3 under a $\$1000$ BOM limit represents a high risk with current COTS/FDM BLDC technologies.
*   **Decision:** S3 is deferred to Phase 2 (or a future scale-up) and is excluded from the Phase 1 implementation baseline.

### Next Step for Human Decision (ADR-0005):
Propose **Scheme S2** as the primary physical architecture for the `sint` interface. Detail the joint layout, link segment joins, and belt channel access covers in the upcoming ADR-0005, pending final maintainer acceptance.
