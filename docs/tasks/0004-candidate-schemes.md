...
| **J4** | Forearm Roll/Pitch (J4_pitch) | `in_joint` | `distal` | ~3–5 |
...
graph TD
    ...
    J2 --- L2[Link 2: Upper Arm ~0.3m]
    L2 --- J3[J3: Elbow Pitch]
    J3 --- L3[Link 3: Forearm ~0.3m]
    L3 --- J4[J4: Wrist Pitch]
    J4 --- J5[J5: Wrist Yaw/Pitch]
    J5 --- J6[J6: Wrist Roll]
    J6 ==>|Quick-Change EE| EE[Tool / Gripper / Dock Latch]
```

### 1.4 Relocatable-Base Story
- **Sequence:** The arm operates docked at Dock A. To relocate to Dock B, the EE attaches a temporary support strut / latch extension to Dock B or uses a specialized heavy walk-end adapter. Once Dock B latches and verifies power/data integrity, Dock A releases, and the arm swings around using Dock B as its new base.
- **Cantilever Load Carrying:** Cantilever loads during stepping are carried entirely by the proximal base dock (Dock A during transit start, Dock B upon locking). The distal wrist (J4–J6) is light (~1–3 N·m) and is **never** used as a structural cantilever pivot for the main arm body; stepping requires dual-dock overlap or an auxiliary structural latch on the base segment.
...
- **Reach:** Nominal arm length ~0.65 m (Upper Arm 0.30 m + Forearm 0.30 m + Wrist 0.05 m), stretch up to 0.8 m.
- **Dock Spacing:** 2 docks spaced ~0.7 m apart. Dock A services the Bambu Lab A1 mini bed and filament bay. Dock B services the assembly table and tool-bay cluster. Single-step reach covers both stations cleanly without exceeding Link 1–3 Segment length constraints (all printed components ≤ 175 mm).
...
relocatable: true
ends: asymmetric
notes: "Pure in-joint FOC modules; transit battery on Link 1; wrist not sole walk support"
```
...
- **Cantilever Load Carrying:** Because the wrist group (J4–J6) is ultralight and driven remotely by cascaded belts, it cannot withstand main arm cantilever bending loads. Stepping is executed by bringing a dedicated base-segment auxiliary docking latch into contact with Dock B (or docking via a dual-latch base bracket), keeping all heavy cantilever moments on the main proximal joints (J1/J2) and dedicated structural latches.
...
- **Reach:** Nominal 0.70 m (Link 2: 0.32 m, Link 3: 0.30 m, Wrist: 0.08 m), maximum stretch 0.85 m.
- **Dock Spacing:** ~0.8 m center-to-center between Dock A (Printer service zone) and Dock B (Table & Tool Bay). Low forearm inertia allows fast scanning and assembly pick-and-place cycles across both zones. All link shells segmented into ≤ 175 mm printable sub-blocks.
...
relocatable: true
ends: asymmetric
notes: "Default placement policy; cascaded drives in L2/L3 with agent-accessible belt channels"
```
...
S3 is a 7-DoF symmetric dual-ended manipulator featuring identical, high-torque universal docking/EE interfaces on both ends. The joint topology is fully symmetric (Yaw-Pitch-Pitch-Pitch-Pitch-Yaw-Roll or Roll-Pitch-Yaw / Pitch / Yaw-Pitch-Roll), allowing either end to serve as the stationary base or the active end-effector. Both terminal joint clusters (J1–J2 and J6–J7) carry high-torque proximal-class actuators so that either end can support full cantilever arm loads during relocatable walking. S3 natively operates under Power Scenario B (dock-only continuous power with dual-dock hot-handoff reconnection).
...
| **J3** | End-A Yaw/Pitch (J3_pitch) | `in_joint` | `proximal` | ~15–20 |
| **J4** | Elbow Pitch (J4_pitch) | `in_joint` | `proximal` | ~10–15 |
| **J5** | End-B Pitch (J5_pitch) | `in_joint` | `proximal` | ~15–20 |
| **J6** | End-B Pitch/Yaw (J6_pitch) | `in_joint` | `proximal` | ~20–25 |
...
- **Reach:** Nominal 0.80 m (Link 1: 0.35 m, Link 2: 0.35 m, End clusters: 0.10 m total), maximum stretch 1.0 m.
- **Dock Spacing:** ~0.8–1.0 m between Dock A and Dock B. The extended reach and symmetric walking logic allow traversing longer distances between print beds, storage tables, and tool stations. Printable link shells are split into ≤ 175 mm segments.
...
- **ADR-0004 Compliance:** Implements "Relocatable base / walking-arm logic (scaled Canadarm pattern)" Retain/Pattern decision. Higher terminal joint mass is accepted as a trade-off for zero-battery operation and full end symmetry.
...
relocatable: true
ends: dual_ended
notes: "Symmetric Canadarm-style walking arm; dual high-torque ends support full cantilever walk; Scenario B hot-handoff"
```
